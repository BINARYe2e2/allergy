...
async def analyze_menu_image(image_bytes: bytes, mime_type: str = "image/jpeg", profile_text: str = "") -> MenuAnalysisResponse:
    ...
    # 사용자의 구조화된 프로필 조건(금지 식재료 / 예외 및 특이사항)을 철저히 반영하는 시스템 프롬프트
    system_prompt = f"""
당신은 전 세계의 다국어 메뉴판 및 성분 표기판을 분석하는 최고 수준의 임상 영양사 및 알레르기 안전 전문가입니다.
제공된 메뉴판 또는 원산지 표기판 이미지를 철저히 분석하여 알레르기 위험 요소를 추출하세요.

[사용자 맞춤형 알레르기 및 식이 가이드]
"{profile_text if profile_text else '등록된 특별한 알레르기 조건이 없습니다.'}"

[분석 및 출력 필수 지시사항]
1. 사용자가 입력한 [주의/금지 식재료]는 무관용 원칙으로 엄격하게 위험 요소를 판별하고, [예외 및 특이사항]에 언급된 내용(예: 오류 허용, 조리 방식 예외 등)은 반드시 반영하여 안전/주의 여부를 유연하게 판정하세요.
2. 메뉴의 조리 과정(육수, 소스, 숨겨진 양념 등)까지 유추하여 각 메뉴별로 **구체적인 판정 근거(reasoning)**를 상세히 작성하세요.
    ...
    for model_name in candidate_models:
        try:
            response = client.models.generate_content(
                ...
                contents=[
                    image,
                    "이 메뉴판/원산지 표기 이미지에서 메뉴와 사용자 프로필 맞춤 알레르기 위험 요소를 추출해줘."
                ],
